/* WallKeep visual scanner v13 - fast visual first, OCR only as fallback */
(function(){
...
async function loadIndex(){
  ...
  const r=await fetch('data/op16-index.json?v=13',{cache:'force-cache'});
...
async function embedding(el){const m=await loadModel();const t=m.infer(el,true);const a=l2(Array.from(await t.data()));t.dispose();return a}
...
function bottomCrop(canvas){
  const out=document.createElement('canvas');
  const y=Math.floor(canvas.height*.48),h=canvas.height-y;
  out.width=canvas.width;out.height=h;
  out.getContext('2d',{willReadFrequently:true}).drawImage(canvas,0,y,canvas.width,h,0,0,out.width,out.height);
  return out
}
...
async function identify(canvas){
  ...
  showDebug(rows,text,'VISUALE + OCR MIRATO');
  return rows
}
scanDetected=async function(d){if(vBusy||vLocked)return;vBusy=true;try{if(game!=='onepiece')throw Error('Scanner visuale v13 in test solo su One Piece OP16');const frozen=warpDetected(d);$('#scanStatus').textContent='Fotogramma congelato • riconosco…';await preload();const rows=await identify(frozen);if(!rows.length)throw Error('Nessun candidato');const card=rows[0],clean={...card};delete clean.vec;delete clean._visual;delete clean._boost;delete clean._score;vLocked=true;const last=lastCard();if(last&&same(last.name,clean.name)){duplicate(clean,card._score);return}addReview(clean,card._score);$('#scanStatus').innerHTML='<span class=good>✓ '+esc(clean.name)+' • '+esc(clean.collector)+' aggiunta. Rimuovi la carta.</span>';stableFrames=0;lastQuad=null}catch(e){vLocked=true;$('#scanStatus').innerHTML='<span class=bad>'+esc(e.message)+'</span>'}finally{vBusy=false}}
const detector=detectCard;
startDetection=function(){
  clearInterval(detectTimer);
  if(game==='onepiece')preload().catch(e=>{$('#scanStatus').innerHTML='<span class=bad>'+esc(e.message)+'</span>'});
  detectTimer=setInterval(()=>{const d=detector();if(d){missing=0;lastDet=d;seen++;if(!vLocked)$('#scanStatus').textContent=seen>=3?'Carta stabile • acquisisco…':'Carta rilevata…';if(rapid&&!vBusy&&!vLocked&&seen>=3)scanDetected({...d,stable:true})}else{missing++;seen=0;if(missing<6&&lastDet){if(!vLocked)$('#scanStatus').textContent='Mantengo aggancio…';return}lastDet=null;if(missing>=6){vLocked=false;document.querySelector('#duplicateBtn')?.remove();lastCode=''}if(!vLocked)$('#scanStatus').textContent='Cerco la carta…'}},220)
};
})();