...
function mapCard(x,code){return {name:x.card_name||x.name||code,setName:x.set_name||x.card_set||'OP16',setCode:'OP16',collector:code,image:x.card_image||x.image||'',color:x.card_color||x.color||'',sourceImageId:String(x.card_image_id||'')}}
...
for(const x of Array.isArray(raw)?raw:[]){
  const code=codeOf(x),card=mapCard(x,code);
  const imageKey=String(card.image||'').trim();
  const key=code+'|'+imageKey;
  if(!code||!imageKey||seen.has(key))continue;
  seen.add(key);cards.push(card)
}
cards.sort((a,b)=>a.collector.localeCompare(b.collector)||a.image.localeCompare(b.image));
const variantCount=new Map();
for(const c of cards){
  const n=(variantCount.get(c.collector)||0)+1;variantCount.set(c.collector,n);
  c.variantIndex=n;c.id=`${c.collector}#${n}`
}
console.log('Artwork variants found',cards.length,'unique collector numbers',variantCount.size);
const rows=[];
for(let i=0;i<cards.length;i++){
  ...
  try{
    ...
    img.dispose(); resized.dispose(); emb.dispose();
    rows.push({...c,vec});
    console.log(`${i+1}/${cards.length}`,c.collector,'variant',c.variantIndex,'ok');
...
console.log('Wrote',rows.length,'artwork rows');
